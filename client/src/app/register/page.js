...
export default function RegisterPage() {
  ...
  const { register, isAuthenticated, isLoading, error: apiError, clearError } = useAuth();
  ...
  // Clear store errors on load/unmount
  useEffect(() => {
    clearError();
    return () => clearError();
  }, [clearError]);
    ...
    <div className="min-h-screen relative flex items-center justify-center px-4 bg-slate-950 overflow-hidden select-none">
      ...
      <div className="relative w-full max-w-md animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Get started with habit tracking today</p>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-8 backdrop-blur-xl shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              ...
              <a href="#" className="font-medium text-slate-400 hover:text-slate-300">
                Terms of Service
              </a>{' '}
              and{' '}
              <a href="#" className="font-medium text-slate-400 hover:text-slate-300">
